...
def get_nvdfile_to_db(nvdfilename='',database='/var/db/cvedb'):
    ...
    try:
        ...
        cmd = "./load.sh " + nvdfilename + ' ' + database
        print "cmd: %s" % (cmd,)
        cmd = cmd.split()
        subprocess.call(cmd)
        cmd = "rm " + nvdfilename
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        database = '/var/db/cvedb'

    for year in range(start_year, current_year+1):
        filename = r'nvdcve-2.0-' + str(year) + r'.xml.gz'
        get_nvdfile_to_db(filename,database)

    get_nvdfile_to_db(r'nvdcve-2.0-Recent.xml.gz',database)
    get_nvdfile_to_db(r'nvdcve-2.0-Modified.xml.gz',database)
